...
export default function Collection({params}: {params: {collectionName: string}}) {
    ...
    const getListData = async () => {
        ...
        const fields = modelBody.split('\n').map((field: string) => field.trim()).filter((field: string) => field.length > 0);
        ...
        const parsedFiedsInThreeColumns = parsedFields.map((field: string[])=>[field[0], field[1], field.slice(2).join(' ')]);
    ...
    const handleInputChange = (index: number, fieldIndex: number, value: string) => {
        ...
        <div className="flex w-full h-screen-minus-navbar-topbar">
            ...
            <div className="w-full px-6 overflow-scroll">
                ...
                <Section name="Basic Details" isExpanded={true}>
                    ...
                    <Button variant={"outline"} className="mt-5" onClick={()=>setCollectionData([...collectionData, ["", "", ""]])}>Add Field</Button>
                </Section>
            </div>
            )}
        </div>
